<div class="container-fluid bg-white mt-5">
    <div class="row">
        <div class="col-lg-4 p-4">
            <h3 class="h-font fw-bold fs-3">2T HOTEL</h3>
            <p>

            </p>
        </div>
        <div class="col-lg-4 p-4">
            <h5 class="mb-3">Links</h5>
            <a href="#" class="d-inline-block mb-2 text-dark text-decoration-none">Trang Chủ</a><br>
            <a href="#" class="d-inline-block mb-2 text-dark text-decoration-none">Giới thiệu</a>
            <a href="#" class="d-inline-block mb-2 text-dark text-decoration-none">Hệ Thống Phòng</a><br>
            <a href="#" class="d-inline-block mb-2 text-dark text-decoration-none">Tiện ích</a><br>
            <a href="#" class="d-inline-block mb-2 text-dark text-decoration-none">Liên hệ</a><br>

        </div>
        <div class="col-lg-4 p-4">
            <h5 class="mb-3">Follow us</h5>
            <a href="#" class="d-inline-block text-dark text-decoration-none md-2">
                <i class="bi bi-facebook me-1"></i> Facebook
            </a><br>
            <a href="#" class="d-inline-block text-dark text-decoration-none md-2">
                <i class="bi bi-youtube me-1"></i> Youtube
            </a><br>
            <a href="#" class="d-inline-block text-dark text-decoration-none ">
                <i class="bi bi-instagram me-1"></i> Instagram
            </a><br>
        </div>
    </div>
</div>